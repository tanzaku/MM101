...
public class WorldCupLineupVis {
  ...
  private void generateTestCase(long seed) {
    ...
    for (int i = 0; i < numGroups; i++) {
      ...
      while (r.nextDouble() < 0.5) {
        ...
        if (currentGroup.contains(testMember))
          break;
      ...
      for (int j = 0; j < groupMembers[i].length; j++)
        groupMembers[i][j] = currentGroup.get(j);
      groups[i] = "" + groupMembers[i][0];
      for (int j = 1; j < groupMembers[i].length; j++)
        groups[i] += "," + groupMembers[i][j];
  ...
  private int createCurrentLineup() {
    ...
    for (int i = 0; i < 10; i++) {
      totalAtk += calcAtk[i];
      totalDef += calcDef[i];
      writeError("Position " + i + " (" + position[i] + "): " + lineup[i] + ": Atk = " + calcAtk[i] + ", Def = "
          + calcDef[i] + ", Agg = " + calcAgg[i]);
    }
    ...
    for (int i = 0; i < 10; i++) {
      ...
      if (r.nextInt(100) < calcAgg[i]) {
        if (carded[lineup[i]]) {
          ...
          writeError(
              "Player " + lineup[i] + " in position " + i + " was shown the yellow card, but is still in the game.");
